...
#define DRCCTLIB_EXIT_PROCESS(format, args...)                                           \
    DRCCTLIB_CLIENT_EXIT_PROCESS_TEMPLATE("DrDeadSpy.cpp", format, \
                                          ##args)
...
{
    #ifdef ARM_CCTLIB
    char name[MAXIMUM_PATH] = "arm.DrDeadSpy.out.";
#else
    char name[MAXIMUM_PATH] = "x86.DrDeadSpy.out.";
#endif
...
ClientExit(void)
{
    ...
    if (!dr_raw_tls_cfree(tls_offs, INSTRACE_TLS_COUNT)) {
        DRCCTLIB_EXIT_PROCESS(
            "ERROR: DrDeadSpy dr_raw_tls_calloc fail");
    }
    if (!drmgr_unregister_thread_init_event(ClientThreadStart) ||
        !drmgr_unregister_thread_exit_event(ClientThreadEnd) ||
        !drmgr_unregister_tls_field(tls_idx)) {
        DRCCTLIB_PRINTF("ERROR: DrDeadSpy failed to "
...
{
    dr_set_client_name("DynamoRIO Client 'DrDeadSpy'",
                       "http://dynamorio.org/issues");
    ClientInit(argc, argv);

    if (!drmgr_init()) {
        DRCCTLIB_EXIT_PROCESS("ERROR: DrDeadSpy "
